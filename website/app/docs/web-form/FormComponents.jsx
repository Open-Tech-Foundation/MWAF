...
export function FormField(props) {
  ...
  return (
    <div className="flex flex-col gap-1.5 mb-5 group">
      <label className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1 group-focus-within:text-accent transition-colors">
        {label}
      </label>
      <input
        name={name}
...
export function FormStatus({ form }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
      <div className={isValid ? 'flex items-center justify-center gap-2 p-2.5 rounded-xl border transition-all duration-500 bg-emerald-50 border-emerald-200 text-emerald-600 shadow-sm' : 'flex items-center justify-center gap-2 p-2.5 rounded-xl border transition-all duration-500 bg-[var(--bg-surface)] border-[var(--border)] text-[var(--text-muted)]'}>
        <span className="text-[10px] font-black uppercase tracking-widest">{isValid ? "Valid" : "Invalid"}</span>
      </div>
      <div className={isChanged ? 'flex items-center justify-center gap-2 p-2.5 rounded-xl border transition-all duration-500 bg-blue-50 border-blue-200 text-blue-600 shadow-sm' : 'flex items-center justify-center gap-2 p-2.5 rounded-xl border transition-all duration-500 bg-[var(--bg-surface)] border-[var(--border)] text-[var(--text-muted)]'}>
        <span className="text-[10px] font-black uppercase tracking-widest">{isChanged ? "Changed" : "Clean"}</span>
      </div>
      <div className={isTouched ? 'flex items-center justify-center gap-2 p-2.5 rounded-xl border transition-all duration-500 bg-amber-50 border-amber-200 text-amber-600 shadow-sm' : 'flex items-center justify-center gap-2 p-2.5 rounded-xl border transition-all duration-500 bg-[var(--bg-surface)] border-[var(--border)] text-[var(--text-muted)]'}>
        <span className="text-[10px] font-black uppercase tracking-widest">{() => isTouched ? "Touched" : "Untouched"}</span>
      </div>
      <div className={isSubmitting ? 'flex items-center justify-center gap-2 p-2.5 rounded-xl border transition-all duration-500 bg-purple-50 border-purple-200 text-purple-600 shadow-sm' : 'flex items-center justify-center gap-2 p-2.5 rounded-xl border transition-all duration-500 bg-[var(--bg-surface)] border-[var(--border)] text-[var(--text-muted)]'}>
  ...
  return (
    <div className="flex flex-col gap-1.5 min-w-[100px]">
      ...
      <div className="relative group">
        ...
        <div className="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none text-[var(--text-muted)] group-hover:text-accent transition-colors">
    ...
    <div className="flex flex-col gap-6 sticky top-8">
      <div>
        <div className="flex items-center gap-2 mb-3 ml-1">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)]">Live Values</p>
        </div>
        <pre className="text-[11px] font-mono bg-slate-900 p-5 rounded-2xl border border-slate-800 text-blue-300 overflow-auto max-h-[300px] leading-relaxed shadow-inner">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3 ml-1">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)]">Validation State</p>
        </div>
        <pre className="text-[11px] font-mono bg-slate-900 p-5 rounded-2xl border border-slate-800 text-amber-300 overflow-auto max-h-[300px] leading-relaxed shadow-inner">
      ...
      <div className="space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 px-1">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black text-[var(--text-main)] tracking-tight leading-tight">Basic Account</h2>
              <p className="text-[var(--text-muted)] text-xs font-medium tracking-wide">Enter your core credentials</p>
            </div>
          </div>
        ...
        <section className="bg-[var(--bg-main)] p-8 rounded-3xl border border-[var(--border)] shadow-sm">
        ...
        <form onsubmit={form.handleSubmit(async (v) => {
          ...
          <div className="flex gap-4 mt-8">
            ...
            <button
              ...
              disabled={!canSubmit}
              className={canSubmit
                ? 'flex-[2] py-3.5 text-white rounded-xl transition-all duration-500 font-bold tracking-wide uppercase text-[11px] shadow-lg bg-accent hover:opacity-90 shadow-accent/20'
                : 'flex-[2] py-3.5 text-[var(--text-muted)] rounded-xl bg-[var(--bg-surface)] cursor-not-allowed grayscale'
              }
            >
          ...
          {() => isSubmitted && (
            <div className="mt-6 p-3 rounded-xl bg-emerald-50 border border-emerald-100 text-center text-xs font-bold text-emerald-600">
      ...
      <div className="space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 px-1">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black text-[var(--text-main)] tracking-tight leading-tight">Advanced Profile</h2>
              <p className="text-[var(--text-muted)] text-xs font-medium tracking-wide">Nested data & preferences</p>
            </div>
          </div>
        ...
        <form onsubmit={form.handleSubmit(async (v) => {
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <label className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1">Skills & Expertise</label>
              <button type="button" onclick={addSkill} className="text-[10px] font-bold text-accent hover:opacity-80 flex items-center gap-1 transition-opacity">
                  ...
                  <div className="flex-1">
                    <input
                      ...
                      className={form.errors.skills?.[index]
                        ? "w-full px-4 py-2.5 rounded-xl border bg-red-500/5 border-red-500/50 text-[var(--text-main)] text-sm outline-none"
                        : "w-full px-4 py-2.5 rounded-xl border bg-[var(--bg-surface)] border-[var(--border)] text-[var(--text-main)] text-sm outline-none focus:border-accent/50 transition-all"}
                    />
                  </div>
                  <button
                    type="button"
                    onclick={() => removeSkill(index)}
                    className="p-2.5 rounded-xl border border-[var(--border)] text-[var(--text-muted)] hover:text-red-500 hover:bg-red-50 transition-all"
                  >
          ...
          {() => isSubmitted && (
            <div className="mt-6 p-3 rounded-xl bg-purple-50 border border-purple-100 text-center text-xs font-bold text-purple-600">
